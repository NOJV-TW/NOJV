<script lang="ts">
  ...
  function scheduleError(error: unknown, field: "startsAt" | "endsAt") {
    const raw = Array.isArray(error) ? error[0] : error;
    if (typeof raw !== "string" || !raw) return null;
    if (raw.includes("endsAt must be later than startsAt")) {
      return m.examCreate_scheduleOrder();
    }
    const label = field === "startsAt" ? m.examCreate_startsLabel() : m.examCreate_endsLabel();
    if (raw.includes("Too small") || raw.includes("expected")) {
      return m.examCreate_scheduleRequired({ field: label });
    }
    return m.examCreate_scheduleInvalid({ field: label });
  }
</script>
...
<PageContainer width="form" class="space-y-6">
  ...
  <form method="POST" use:enhance class="animate-in animate-in-1 space-y-5">
    ...
    <div
      class="space-y-8 rounded-xl border border-border-subtle bg-[color:var(--color-panel)] p-5 shadow-rest backdrop-blur"
    >
