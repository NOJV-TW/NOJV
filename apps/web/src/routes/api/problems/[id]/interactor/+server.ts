...
export const POST: RequestHandler = writeApiHandler(async (event) => {
  ...
  if (!(file instanceof File)) {
    error(400, "No interactor script provided");
  }
  if (file.size > MAX_SIZE) {
    error(413, `Interactor script too large (max ${String(MAX_SIZE / (1024 * 1024))} MB)`);
  }
  ...
  const result = await problemDomain.setProblemInteractor(actorContext, problemId, {
    content,
