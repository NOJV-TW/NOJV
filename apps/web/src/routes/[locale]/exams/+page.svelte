...
<div class="space-y-6">
  ...
  {#if data.items === null}
    <p class="text-sm text-[color:var(--color-muted)]">{$t("examsList.signInRequired")}</p>
  {:else if data.items.length === 0}
    <p class="text-sm text-[color:var(--color-muted)]">{$t("examsList.empty")}</p>
  {:else}
    <section class="grid gap-4">
      {#each data.items as a (`${a.courseSlug}-${a.slug}`)}
        <a
          ...
          href="/{locale}/courses/{a.courseSlug}/exams/{a.slug}"
        >
          <div>
          ...
          <div>
            <p class="text-sm text-[color:var(--color-muted)]">{$t("examsList.opens")}</p>
            <p class="mt-1 text-sm">{new Date(a.opensAt).toLocaleDateString(locale)}</p>
          </div>
          <div>
            <p class="text-sm text-[color:var(--color-muted)]">{$t("examsList.due")}</p>
