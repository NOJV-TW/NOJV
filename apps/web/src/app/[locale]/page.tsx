...
export default async function LocaleHomePage({
  ...
  const [tHome, tAuth] = await Promise.all([getTranslations("home"), getTranslations("auth")]);
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-2">
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-[color:var(--color-border)]">
                <th className="pb-2 pr-4 font-medium text-[color:var(--color-muted)]">
                  {tHome("date")}
                </th>
                <th className="pb-2 font-medium text-[color:var(--color-muted)]">
                  {tHome("content")}
                </th>
              </tr>
            </thead>
      ...
      {!isLoggedIn ? (
        <section
          className={`${shellClassNames.cardStrong} flex flex-col items-center justify-center gap-4 px-6 py-10 text-center`}
        >
              ...
              >
                <div>
                  <p className="text-xs uppercase tracking-widest text-[color:var(--color-muted)]">
                    {a.courseTitle} ·{" "}
                    {a.type === "assignment" ? tHome("assignment") : tHome("exam")}
                  </p>
