...
<div
  class="flex h-full min-h-[240px] flex-col gap-1 rounded-md border border-border-subtle bg-[color:var(--color-panel)] p-2"
>
    ...
    <ul class="space-y-0.5">
      {#each files as file, index (`file-${String(index)}-${file.path}`)}
        <li>
          <button
            type="button"
            class="flex w-full items-center justify-between rounded-md px-2 py-1.5 text-left text-body-sm transition-[background-color,color] duration-fast ease-out-soft hover:bg-accent {selectedIndex ===
            index
