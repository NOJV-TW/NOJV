<script lang="ts">
  ...
  interface Props {
    data: ContestResults;
    matrix: contestDomain.ContestSubmissionsMatrix;
    contestId: string;
  }

  let { data, matrix, contestId }: Props = $props();
</script>

<AssessmentGradesTab
  {matrix}
  stats={data}
  csvDownloadName={`contest-${contestId}-grades.csv`}
  dataSlot="contest-grade-matrix"
  labels={{
