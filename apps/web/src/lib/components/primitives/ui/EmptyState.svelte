...
{#if isOnboarding}
  <div class={cn("flex flex-col items-center justify-center py-12 text-center", className)}>
    ...
    <h3 class="text-[length:var(--text-title-lg)] leading-tight mt-5 [text-wrap:balance]">
      {title}
    </h3>
    {#if description}
      <p
...
{:else}
  <div class={cn("flex flex-col items-center justify-center py-16 text-center", className)}>
    <div class="flex h-14 w-14 items-center justify-center rounded-lg bg-muted/60">
      <Icon class="h-7 w-7 text-muted-foreground/70" />
    </div>
    <h3 class="text-[length:var(--text-title)] leading-tight mt-4 [text-wrap:balance]">
      {title}
    </h3>
    {#if description}
      <p
