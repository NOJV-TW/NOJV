...
export async function isContextClosed(context) {
    const now = Date.now();
    switch (context.type) {
        case "assignment": {
...
export async function assertContextClosed(context) {
    if (!(await isContextClosed(context))) {
        throw new ConflictError("This context is still open; grading is only available after it closes.");
    }
}
