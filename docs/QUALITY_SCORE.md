...
| Product specification      | A-    | PRODUCT_SENSE.md + per-feature acceptance specs under `docs/specs/` (assignments, exams, contests, copy-course, proctoring, plagiarism, editorials, dashboard). | Promote inline `editorialSubmitSchema` into `@nojv/core`; fix dashboard recommendation drift. |
...
| Test coverage              | C+    | Vitest unit/integration and Playwright E2E configured (45 unit files / 387 tests) but no coverage threshold and integration gaps remain.                        | Add coverage thresholds and missing integration tests.                                        |

## Doc Drift Status

- 2026-04-20 spec backfill: added `docs/specs/plagiarism.md`, `docs/specs/editorials.md`, `docs/specs/dashboard.md`. Dashboard spec flags one live drift — `PRODUCT_SENSE.md § User Dashboard` lists "problem-solving recommendations" but no recommender is implemented; editorial spec flags the "Solve this problem first to view editorials." error string being reused on POST.
