...
| Change problem, course, exam, contest, or submission rules | `packages/application/src/<domain>/`; problem writes live in `problem/mutations/{records,publishing,judge-config}.ts`, submission reads in `submission/{details,history,judge-context}.ts` | [Product Sense](product/PRODUCT_SENSE.md), matching [feature spec](specs/), [Architecture](architecture/ARCHITECTURE.md)       | `tests/unit/application/`, `tests/integration/application/`                                                |
