...
| Change authentication or other permissions           | `apps/web/src/lib/auth.server.ts`, `packages/application/src/<domain>/`                                                             | [Security](operations/SECURITY.md), [Threat Model](operations/THREAT_MODEL.md), [login spec](specs/login-security.md)          | `pnpm exec vitest run --project unit tests/unit/web/`; `pnpm exec vitest run --project integration tests/integration/http/`; `pnpm test:e2e` |
