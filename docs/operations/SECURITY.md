...
- The Kubernetes executor refuses **tarball-source** Advanced Mode requests with a System Error verdict — the K8s path cannot `docker load` a TA-supplied tarball. **Registry-source** advanced images run on K8s as Jobs with `cap-drop ALL`, `allowPrivilegeEscalation: false`, read-only rootfs, pod-level `seccompProfile: RuntimeDefault`, and `runAsNonRoot: true` (uid 10001) on **both** the run and grade pods. On the **Docker** backend the grade phase runs as root (no `--user`) and the grade container has unrestricted network egress on both backends — the TA grader image is trusted and the answer-bearing grade container is isolated from the student's run container. The residual egress-exfiltration risk is documented in [THREAT_MODEL](THREAT_MODEL.md) as an accepted risk.
