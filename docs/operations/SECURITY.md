...
- **Two-tier admin (sudo model).** OAuth or password sign-in always starts an `admin` account with ordinary effective permissions. `getActorContext` computes an **effective** `platformRole` (`resolveEffectivePlatformRole`) that downgrades a de-elevated admin to `student`, so every existing `platformRole === "admin"` check stays off until explicit elevation. Every admin must enable 2FA and complete a fresh TOTP or passkey step-up before `POST /api/admin-mode` stores the per-session elevation in Redis. Turning admin mode off deletes both the elevation and its MFA marker, so switching back requires another step-up. Do NOT read `sessionUser.platformRole` for power decisions — use `actor.platformRole` (effective) so the boundary is honoured.
- **Super admin** = `isSuperAdmin && platformRole === "admin"`. Only an elevated super admin may grant/revoke the `admin` role (`updateUserRole` enforces it) or disable another super admin. Superadmin sessions retain the shorter maximum age, but use the same explicit 2FA elevation flow as every other admin. Read `sessionUser.isSuperAdmin` only for superadmin-specific authorization in code that has already required the effective admin role.
