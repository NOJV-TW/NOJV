...
8. Capacity-enabled runs are ordered round-robin by student, with FIFO submissions per student and at most one active permit per student. Foreground student submissions are admitted before background rejudge or recovery executions at every permit decision, including a student's own pending rejudges, so a bulk rejudge never blocks live judging for longer than one in-flight wave. A completed wave re-enters admission. Prepared unfinished attempts are bounded independently from total accepted submissions; accepting 100 submissions does not create 100 artifact PVCs at once. FIFO wakeups and first-permit replies are durable signals; neither a wake nor an expired wait grants resources or releases a lease. The coordinator retains undelivered notifications and periodically reconciles database-only cancellations; every resumed execution rechecks its persisted FIFO position and owner.
