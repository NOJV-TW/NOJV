...
- Activate off-site backups in production: Postgres barman archive and the object-store mirror (rclone, `nojv` and `nojv-registry`) to Cloudflare R2's free tier, then run a restore drill. The single-machine overlay refuses to render until real destinations are supplied. See OPS-06 and [Backup & Restore](../runbooks/backup-restore.md#enable-the-off-host-mirror). Steps await owner confirmation in the [production write verification plan](../superpowers/plans/2026-09-26-production-write-verification.md). R2 is chosen but not yet enabled on the account; production has never taken a base backup (`cnpg_collector_last_available_backup_timestamp` is 0).
