...
- Legacy judge removal, final stage: nothing starts `submissionJudgeWorkflow` or `rejudgeWorkflow`, but both stay registered for replay. Once production shows no running legacy executions (`temporal workflow list --namespace <ns> --query "WorkflowType IN ('submissionJudgeWorkflow', 'rejudgeWorkflow') AND ExecutionStatus = 'Running'"` is empty) and no non-terminal pre-durable work rows (`SELECT kind, status, count(*) FROM "DurableWork" WHERE status IN ('pending', 'leased') AND (kind = 'submission.judge.dispatch' OR (kind = 'submission.rejudge.dispatch' AND (payload->>'prepared') IS DISTINCT FROM 'true')) GROUP BY kind, status;` returns no rows), delete both workflows and their helpers in `apps/worker/src/workflows/`, the legacy activities in `apps/worker/src/activities/judge.ts`, the non-`prepared` progress/cancel branches in `packages/application/src/submission/rejudge-control.ts` with the `queryRejudgeProgress` / `cancelRejudge` port methods they use, the legacy-owner branch of `sweepStaleSubmissions`, and the no-op `submission.judge.dispatch` / `submission.rejudge.dispatch` handlers in `apps/worker/src/activities/durable-work-registry.ts`.
