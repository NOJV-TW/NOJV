...
| Security guidance          | B+    | Handling rules, sensitive data, threat model cover all surfaces. CI runs CodeQL SAST + blocking `pnpm audit` gate (0 high/critical). DOMPurify transitives cleared via a `monaco-editor>dompurify` override; only residual is `@hono/node-server` in Prisma dev-only tooling. | Re-clear the `@hono/node-server` residual when Prisma bumps `@prisma/dev` (dev-only, not in the runtime path).               |
