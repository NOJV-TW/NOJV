...
| `/api/release`                               | GET                      | Public exact-path release identity; version and source SHA only, no runtime or user data |
...
- **Recycled school mailbox**: A school reassigns a graduate's address to a new student, who signs in with the new Google account behind that address. _Mitigation_: `disableImplicitLinking` — an unknown provider identity is never attached to an existing account by email; the sign-in stops at `/signin` and only an already-authenticated session can link a new provider. `User.email` moves only through `changeEmail`, which confirms from the _current_ mailbox before verifying the new one and, once the account has a factor, additionally requires the security-settings unlock — so a stolen session cannot redirect the security codes.
...
- **Reading a previous student's code on a shared lab computer**: Editor drafts autosave to the browser's `localStorage`, so the next student at the same machine could open DevTools and read them. _Mitigation_: drafts are stored under `nojv:draft:v2:<userId>:…` and sealed with AES-GCM using a per-user key (`HMAC-SHA256(BETTER_AUTH_SECRET, "code-draft:<userId>")`) that the `(app)` layout hands only to that user's session; the storage key is bound as additional authenticated data, so a draft copied to another key does not open. Legacy plaintext `nojv:draft:v1:` practice/assignment/contest drafts are re-sealed for the first user who opens them and then removed. _Gap_: legacy exam drafts are deliberately left in place (adopting them would hand one student's answers to the next) until cleared; key metadata (user id, context, problem) is visible.
...
- Public probes return only `{ alive: boolean }` or `{ ready: boolean }`; `/api/release` returns only the release tag and source SHA. Their exact raw paths bypass token auth, session loading, CSRF, and rate limiting so external monitoring can observe them; request IDs, Cloudflare policy, and security headers still apply. Internal topology (`postgres`/`redis`/`temporal` status) is admin-only via `/api/admin/healthz`, which requires `requireApiAuth` + `platformRole === "admin"`. Worker `/healthz` returns the full check breakdown but is only exposed inside the cluster.
...
- **Exposed admin endpoints**: Health check or admin routes accessible without auth. _Mitigation_: `/api/livez` and `/api/readyz` are intentionally public and expose only one boolean each; `/api/release` exposes only version identity. Detailed per-subsystem status is admin-only at `/api/admin/healthz`. Admin routes require `requirePlatformRole(actor, "admin")`. Temporal UI (port 8080) is published on `127.0.0.1` only and must additionally be kept off any public interface by the host firewall in production (it has no auth of its own).
