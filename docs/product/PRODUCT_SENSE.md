...
- `User.email` is fixed to the signup address and receives security codes; it cannot be changed. A separately confirmed notification email under settings → notifications receives everything else and falls back to the login email when unset or unconfirmed.
