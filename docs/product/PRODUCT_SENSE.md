...
- Problem creation: any email-verified user (including students) may create, own, and publish private problems. Teachers and admins may publish public problems directly. Platform students who are active course staff (including TAs) must submit an admin-reviewed publication request for a public copy of a private problem. Any private-problem owner may separately grant one-time `adminMayPublish` consent; this remains a distinct admin action and is not a review request, queue, or publication guarantee.
