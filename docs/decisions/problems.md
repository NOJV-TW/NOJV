...
- Rule: every object-storage backend must pass `tests/integration/storage/s3-conformance.test.ts` (atomic `If-None-Match: *`, SHA-256 checksum rejection) before it holds these objects; CI runs it against Versity ([#PR](https://github.com/NOJV-TW/NOJV/pull/PR)).
