...
3. Add language restriction validation:
   ...
   if (contest?.allowedLanguages.length && !contest.allowedLanguages.includes(payload.language)) {
     throw new ForbiddenError("Language not allowed in this contest");
   }
   // For assignment submissions
   if (assessment?.allowedLanguages.length && !assessment.allowedLanguages.includes(payload.language)) {
