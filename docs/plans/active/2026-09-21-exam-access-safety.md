...
- Follow-up landed in the same PR: the contest score-override backend is gone. `scoreOverrideContextSchema`, `ScoreOverrideContext`, `/api/overrides`, the worker's score-convergence payload and the audit timeline accept only assignment and exam; contest scoring and the scoreboard stop reading overrides. Migration `20260923000000_contest_score_override_removal` deletes any contest override rows (production holds none), drops `ScoreOverride.userId` and rebuilds `OverrideContextType` as `assignment | exam`.
