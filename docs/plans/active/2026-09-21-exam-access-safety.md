...
- After merging `origin/main` at `bb78d7b6`, full `pnpm ci:verify` passed: 3,616 unit tests, 100 component tests, build, typecheck, lint, formatting and repository guards. The focused submission HTTP suite passed all 13 tests in the separately marked `nojv_test` database, including older matches beyond the first 50 rows, combined account/IP matching, independent clearing, assignment/exam permissions and changed-filter snapshot rejection. Independent source and merge-resolution reviews found no actionable issue. The merge retains mainline summary expansion and locked dependency versions alongside this PR's grading gates and column filters.

## Contest management tabs

- Contest management is the third `AssessmentManageTabs` kind: Problems (default) / Submissions / Results (Grades, Plagiarism, Audit) / Clarifications when allowed / Settings last, persisted in `?tab=`. The bespoke contest tab list, its `primary`/`inspect` split and the "matrix coming soon" placeholders are gone.
- Submissions is the shared live feed. `/api/submissions?context=contest`, `listRecentContextSubmissions`, `listContextSubmissionsPaged` and the queued-rejudge scope accept a contest id; reads are allowed for the organizer or a platform admin (`canManageContest`).
- Results → Grades reuses `AssessmentGradesTab` (statistics, distribution, matrix, CSV) without cell grading. Product decision: contests do not offer score overrides; assignments and exams keep the cell-click override drawer. The standalone grading button is removed on all three detail pages (assignment/exam also landed on main as #494); the `AssessmentHero` actions slot went with it.
- Tests: contest cases in `context-submissions`, `assessment-tab-state`, `assessment-manage-tabs`; new integration coverage for contest-scoped history, queued-rejudge matching and the HTTP guard. Docs: FRONTEND route table, DESIGN assessment surfaces, contests spec.
- Follow-up landed in the same PR: the contest score-override backend is gone. `scoreOverrideContextSchema`, `ScoreOverrideContext`, `/api/overrides`, the worker's score-convergence payload and the audit timeline accept only assignment and exam; contest scoring and the scoreboard stop reading overrides. `ScoreOverride` and `ScoreOverrideAuditLog` now share `SubmissionFeedback`'s shape: `courseMembershipId` is required, `assessmentId` / `examId` are real foreign keys (overrides cascade with their activity), one override per activity, problem and membership, an XOR check, and the audit trail's `studentUserId`. `OverrideContextType` is gone. Migration `20260923000000_score_override_course_activity` moves the columns (production holds no rows) and `20260923000001_score_override_audit_indexes` adds the concurrent audit indexes.
