...
| Time, fuel, output comparison and grading                                        | NOJV selects budgets, host-clock opt-in, comparison and verdict mapping         | Forge's deterministic virtual clock and weighted instruction budget are intentional replay features, not bugs. Preserve their defaults. Wall time remains different from native CPU time.                                                                                                              |
