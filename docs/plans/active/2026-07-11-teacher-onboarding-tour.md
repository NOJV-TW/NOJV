...
| #   | key                      | 路徑                            | 步驟（錨點 → 文案要點）                                                                                                                                                                                                                                                                              |
| --- | ------------------------ | ------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | `teacher-nav`            | `/dashboard`                    | `nav-primary` 歡迎（教師視角）→ `nav-courses`（作業/考試/成績都從課程進——教師導航列沒有作業/考試連結）→ `nav-problems` → `nav-submissions`。**不用 `dashboard-*`**（零活動不渲染）。                                                                                                                 |
| 2   | `teacher-welcome-guide`  | `/dashboard`                    | `welcome-guide` 單步（零活動教師必渲染，CTA 直達建課）。獨立 intro 讓 settings 重播不會吞掉此步。                                                                                                                                                                                                    |
| 3   | `teacher-courses`        | `/courses`                      | `courses-managing` tab 單步（文案引導點過去）。                                                                                                                                                                                                                                                      |
| 4   | `teacher-course-create`  | `/courses`                      | `courses-create` 單步；enrolled tab 上錨點缺席不標 seen，切到管理 tab（goto 觸發重新評估）才發動。                                                                                                                                                                                                   |
| 5   | `teacher-members`        | `/courses/[id]/members`         | `course-tabs`（課程殼層：總覽/作業/考試/成績/成員/分析/設定）→ `members-bulk-add`（**輸入學號**，一行一個或逗號分隔；未註冊也可先加，會建佔位帳號）→ `members-role`（學生/TA）→ 提醒：**請學生用學校信箱（ntnu/ntu/ntust）登入註冊，系統自動以學號連結；用其他信箱註冊的學生需再完成學校信箱驗證**。 |
| 6   | `teacher-problems`       | `/problems`                     | `problems-mine` tab → `problems-create`（標準/進階）。                                                                                                                                                                                                                                               |
| 7   | `teacher-problem-edit`   | `/problems/[id]/edit`           | `edit-rail`（基本資訊先存檔才解鎖測資/評測；測資 ZIP 檔名 `(\d\d)(\d\d)` 自動分 subtask 與配分——上傳區僅在 Testcase 分區存在故併入此步文案）→ `problem-publish`（無測資不能發布是刻意閘門；題號發布時配發）。                                                                                        |
| 8   | `teacher-assignment-new` | `/courses/[id]/assignments/new` | `assignment-picker`（僅已發布題）→ `assignment-schedule`（開放/截止/關閉）→ `assignment-publish`（草稿 vs 發布；草稿僅課程管理者可見）。                                                                                                                                                             |
| 9   | `teacher-monitor`        | `/assignments/[id]`（管理模式） | `#assignment-manage-tab-problems` 單步，文案一次講完管理分頁（提交矩陣/結果/抄襲/提問/稽核；矩陣在非預設分頁無法錨定）。                                                                                                                                                                             |
| 10  | `teacher-gradebook`      | `/courses/[id]/grades`          | `gradebook`（data-slot，每題顆粒度）→ `gradebook-export`（CSV；亦為 TA/學生閘門錨）。                                                                                                                                                                                                                |
