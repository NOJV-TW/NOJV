...
- **Stage 4 — 切讀(進行中)**:**前置閘門 = `reconcileParticipation()`(`pnpm --filter @nojv/db reconcile:participation`)已實作**——read-only 比對 `Participation` vs 舊三表(contest/exam 比 count+score、virtual 比 count;`ok=false` 即有漂移,先跑 backfill)。**部署 Stages 1–3 + backfill 後,先在 prod 跑 reconcile 確認 `ok=true`,才可開始切讀**。
  ...
  - **第一個 read-switch 已落地**:`getContestWorkspaceData` 的 participation 快照(score/penalty/startedAt/status)改讀 `participationRepo.findContestParticipation`(新讀取 primitive)。平行驗證測試 + contests API 測試改走 mirror 路徑。
  - **剩餘 read-switch(逐個小 PR,同模式)**:scoreboard participant list(`contest.participations`/`exam.participations` include,目前分數從 submissions 重算)、exam workspace、各 detail/管理頁的 participant list。每個都先用平行驗證測試守門。
