...
- **Stage 4 — 切讀**:**前置閘門 = `reconcileParticipation()`(`pnpm --filter @nojv/db reconcile:participation`)已實作**——read-only 比對 `Participation` vs 舊三表(contest/exam 比 count+score、virtual 比 count;`ok=false` 即有漂移,先跑 backfill)。**部署 Stages 1–3 + backfill 後,先在 prod 跑 reconcile 確認 `ok=true`,才可開始切讀**(否則在未驗證的 mirror 上切讀有風險)。切讀本身:scoreboard / 各頁的 participation 讀取改走 `Participation`;移除三表的 domain 讀取。
