...
- ⬜ **Task 2.8 / 5b** (full K8s parity for interactive/advanced) — DEFERRED. K8s `execute` still fail-fasts (SE + operator log) for interactive and advanced; those remain **Docker-backend-only**. Safe (no mis-grade / no leak).
