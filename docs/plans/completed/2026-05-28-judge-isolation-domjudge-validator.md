...
- ⚠️ **Task 2.8a** (K8s checker parity — separate validator Job) — code landed. Run pod ConfigMap excludes both expected answer and validator script; a second `judge-<sub>-validate` Job (identical hardening, separate ConfigMap, no student source) compiles the validator and grades per-case via flat-key ConfigMap entries; `mergeCheckerResults` (shared with Docker) decides verdicts. Runner gains a flat-key fallback for `/submission/cases/...` so the validate pod can read its per-case files from a ConfigMap. **Unit coverage** for ConfigMap shape (run + validate), pod-spec hardening parity, the no-leak invariant (no answer/checker key in run pod), and merge orchestration; **real K8s cluster verification PENDING — cluster smoke required before production rollout**.
- ⚠️ **Task 5b** (K8s advanced — registry-source via init+grader+sidecar) — code landed. Tarball-source still fails fast (clearer operator message; no K8s-side `docker load`); registry-source dispatches a Job with a prep `initContainer` (seeds `/workspace` from a ConfigMap-packed payload), the TA grader as the main container (hardening + memory/cpu/ephemeral-storage limits + automountServiceAccountToken=false + sandbox node-pool + NetworkPolicy label), and an `emit-result` **K8s 1.28+ native sidecar** (initContainer w/ `restartPolicy: Always`) that tails `/workspace/output/result.json` and emits it inside marker lines to its stdout — the worker reads sidecar logs and routes through the shared `mapAdvancedResult` / `advancedFallbackResult` (DRY with Docker backend). **Unit coverage** for pod-spec shape, sidecar restartPolicy, hardening, memory mapping, activeDeadlineSeconds formula, tail-script markers, log marker-extraction round-trip (incl. `{missing:true}` sentinel), tarball-source SE message, and finally-cleanup; **real K8s ≥ 1.28 cluster smoke PENDING — native-sidecar termination semantics and per-container log retrieval need cluster verification.**
- ⬜ **Task 2.8** (K8s interactive parity) — STILL DEFERRED. K8s `execute` continues to route interactive through its existing Docker-only guard. Out of scope for 5b.
...
- **Isolation model:** run/check separation, privilege-free, works on both Docker and K8s. NOT per-case containers (throughput) and NOT in-container namespaces (blocked by hardening + GKE userns unreliability). References: DOMjudge `runguard` (privileged supervisor + unprivileged child), IOI `isolate`, `nsjail` — we adopt their _separation principle_ without needing their privilege by splitting run from check across process/container boundaries.
