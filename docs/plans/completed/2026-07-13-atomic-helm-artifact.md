...
### Task 2: Keep deployment values in the chart artifact
...
2. Update the workflow to rewrite exactly one top-level `image.tag`, stage `infra/charts/nojv/values-single-machine.yaml`, and publish only the `deploy` branch.
