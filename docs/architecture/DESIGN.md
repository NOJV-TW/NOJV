...
- **Assignment and exam management** use `AssessmentManageTabs`: Problems (default) / Submissions / Results (Grades, Plagiarism, Audit) / Clarifications when allowed / Settings last. Exams add Proctoring (Students and sign-in, IP records). Both use the same responsive content spacing and submission toolbar; leaf URLs persist through `?tab=`. Settings group basic information and schedule separately, while assessment-specific controls keep their existing permissions.
